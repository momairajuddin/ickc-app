...
function getDeploymentDomain() {
  if (process.env.EXPO_PUBLIC_API_BASE_URL) {
    const parsed = stripProtocol(process.env.EXPO_PUBLIC_API_BASE_URL);
    if (parsed.includes(".replit.app")) {
      console.log(`Using stable domain from EXPO_PUBLIC_API_BASE_URL: ${parsed}`);
      return parsed;
    }
  }
  ...
  console.error(
    "ERROR: No deployment domain found. Set EXPO_PUBLIC_API_BASE_URL, REPLIT_INTERNAL_APP_DOMAIN, REPLIT_DEV_DOMAIN, or EXPO_PUBLIC_DOMAIN",
  );
  process.exit(1);
}
...
async function startMetro(expoPublicDomain) {
  ...
  const apiBaseUrl = process.env.EXPO_PUBLIC_API_BASE_URL || `https://${expoPublicDomain}`;
